...
#复制脚本到/usr/local/bin/zw
cp ./zw.sh /usr/local/bin/zw > /dev/null 2>&1
...
authorization_check() {
    if grep -q '^user_authorization="true"' /usr/local/bin/zw > /dev/null 2>&1; then
        sed -i 's/^user_authorization="false"/user_authorization="true"/' ./zw.sh
    fi
}
...
authorization_false() {
    if grep -q '^user_authorization="false"' /usr/local/bin/zw > /dev/null 2>&1; then
        user_agreement
    fi
}
...
user_agreement() {
    ...
    if [ "$user_input" = "y" ] || [ "$user_input" = "Y" ]; then
        echo "已同意"
        sed -i 's/^user_authorization="false"/user_authorization="true"/' ./zw.sh
        sed -i 's/^user_authorization="false"/user_authorization="true"/' /usr/local/bin/zw
	#安装sudo
        apt install sudo -y
    else
        echo "已拒绝"
...
root_test(){
    clear
    [ "$EUID" -ne 0 ] && echo -e "${red}提示: ${bai}该功能需要root用户才能运行！" && break_end && zw_sh
}
#endregion
...
echo -e "${pink}zw工具箱 【v$version】 Linux首发！"
echo -e "【该工具箱仅适配Ubuntu/Debian系统】"
echo -e "----输入${red}zw${pink}可再次快速启动此脚本----${white}"
echo -e "${pink}============================${white}"
echo "1. 系统相关->"
echo "2. 测试脚本->"
echo "3. 常用工具下载->"
...
case $choice in
  ...
  555)
    ...
    echo "卸载zw工具箱"
    echo -e "${pink}============================${white}"
    echo "将彻底卸载zw工具箱，不影响已安装的功能"
    read -p "确定继续吗？(Y/N): " confirm
    if [[ "$confirm" == "Y" || "$confirm" == "y" ]]; then
        # 执行删除操作
        clear
        rm -f /usr/local/bin/zw
        rm ./zw.sh
        echo "脚本已卸载，祝您生活愉快！"
        exit
    else
